...
import { getCommunicatedJobs } from '@/api/candidate.js'
...
const CommunicatedJobs = ref([])
const Commounited = ref(true)
...
const isEmpty = ref(false) //判断commounited中的数据是否为空
...
onMounted(async () => {
  ...
  if (res !== null && res.data !== null && res.data.length > 0) {
    const processedCommunicatedJobs = processedData(res) //处理getCommunicatedJobs请求的json转义问题
    CommunicatedJobs.value.push(...processedCommunicatedJobs) // 将处理后的数据放到数据列表里
    ...
    <el-row>
      <el-col :span="18" class="col_left">
        <div class="baseJob">
          <ul>
            <li
              v-for="(item, index) in CommunicatedJobs"
              :key="item.id"
