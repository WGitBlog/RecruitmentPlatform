...
import { getInterestsJobs } from '@/api/candidate.js'
import logoImage from '@/assets/a.jpg'
import { getReportList } from '@/api/report'
import { formatDate } from '@/utils/date'
const boosStore = useBoosStore()

const InterestsJobs = ref([])
const Interests = ref(true)
...
const isEmpty = ref(false) //判断Interests中的数据是否为空
...
onMounted(async () => {
  ...
  if (res !== null && res.data !== null && res.data.length > 0) {
    const processedInterestsJobs = processedData(res) //处理getInterestsJobs请求的json转义问题
    InterestsJobs.value.push(...processedInterestsJobs) // 将处理后的数据放到数据列表里
...
const processedData = (
  res //处理getInterestsJobs请求的json转义问题
) =>
  res.data.map((record) => {
    ...
    <el-row>
      <el-col :span="18" class="col_left">
        <div class="baseJob">
          <ul>
            <li
              v-for="(item, index) in InterestsJobs"
              :key="item.id"
              ...
              <el-row :style="{ height: '80px', lineHeight: '80px' }">
                <el-col :span="15">
                  ...
                  <div class="job_price">
                    ...
                    <ul class="price_ul">
                      <li v-for="span in item.jobDescription" :key="span.id">
                        {{ span }}
                      </li>
                    </ul>
                  </div>
            ...
            >
              <div class="report-header">
                ...
                <div class="action-buttons">
                  <el-button type="primary" link @click="handleViewDetails(report)">
                    查看
                  </el-button>
                </div>
              </div>
              <div class="report-content">
                ...
                <p class="report-time">举报时间：{{ formatDate(report.createTime) }}</p>
    ...
    .baseJob {
      ...
      ul {
        ...
        &::-webkit-scrollbar-thumb:hover {
          background-color: #b3b2b2; /* 鼠标悬停时的色 */
        }
        li {
          display: block;
          margin: 5px 0;
