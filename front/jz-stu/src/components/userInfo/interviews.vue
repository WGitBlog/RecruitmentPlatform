...
import { getInterviewsJobs } from '@/api/candidate.js'
...
const InterviewsJobs = ref([])
const Interviews = ref(true)
...
onMounted(async () => {
  ...
  if (res !== null && res.data !== null && res.data.length > 0) {
    const processedInterviewsJobs = processedData(res) // 处理getInterviewsJobs请求的json数据
    InterviewsJobs.value.push(...processedInterviewsJobs) // 将处理后的数据放到数据列表里
    ...
    <el-row>
      <el-col :span="18" class="col_left">
        <div class="baseJob">
          <ul>
            <li
              v-for="(item, index) in InterviewsJobs"
              :key="item.id"
